<template>
  <div>
    ...
    <div>
      <button v-on:click="goEdit">edit</button>
      <button v-on:click="goPrivate">private</button>
    </div>
    <transition v-bind:name="transitionName">
      <router-view class="child-view" name="nestchild" :item="item"></router-view>
    </transition>
  </div>
</template>
<script type="text/javascript">
  export default {
    ...
    methods: {
      ...
      goPrivate() {
        this.$router
          .push({ name: "itemprivate", params: { id: this.item.id } })
          .catch(function(err) {
            console.log("err", err);
          });
      }
