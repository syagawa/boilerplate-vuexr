<template><div><div v-if="isError"><p>error</p><button v-on:click="retry">retry</button></div><div v-if="isLoading"><p>loading</p></div><div v-if="isLoadedWithNoError"><p>I'm Start</p><div><div>{{ sample }}</div><button v-on:click="addStr">add string</button></div><div><button v-on:click="changeSample({newsample: sample + 'direct'})">add string direct</button></div><div><button v-on:click="nextStart">go next start</button></div><div><button v-on:click="goItems">go items</button></div></div></div></template><script type="text/javascript">import { mapState, mapActions, mapGetters, mapMutations } from "vuex";
import api from "../api/getrandom.js";
export default {
  data: function(){
    return {
      name: "start component",
      ...mapState({
        get_my_sample(state){
          return `${this.name} call ${state.sample.str} in DATA`;
        }
      }),
      ...mapGetters({
        currentSample: "sample/currentSample",
        currentItems: "items/currentItems"
      }),
      loading: true,
      errormessage: null
    };
  },
  computed: {
    sample(){
      return this.$store.getters["sample/currentSample"];
    },
    isError(){
      if(this.errormessage){
        return true;
      }
    },
    isLoading(){
      if(this.loading){
        return true;
      }
    },
    isLoadedWithNoError(){
      return !this.isLoading && !this.isError;
    },
    ...mapState({
      mysample(state){
        console.log(state);
        return `${this.sample} call ${state.sample.str} in COMPUTED`;
      }
    }),
    ...mapGetters({
      isAlreadyGotData: "user/isAlreadyGotData"
    })
  },
  methods: {
    nextStart(){
      if(this.$router.currentRoute.name === "start2"){
        this.$router.push({path: "start"});
      }else{
        this.$router.push({path: "start2"});
      }
    },
    goItems(){
      this.$router.push({name: "items"});
    },
    addStr(){
      const str = this.sample + "add";
      this.changeSample({ newsample: str });
    },
    getData(){
      const self = this;
      api.getData()
        .then(function(data){
          self.setData(data);
          self.loading = false;
          self.errormessage = null;
        })
        .catch(function(err){
          console.log(err);
          self.loading = false;
          self.errormessage = err;
        });
    },
    retry(){
      this.getData();
    },
    ...mapState({
      getMyName(state){
        return `${this.name} call ${state.name} in METHOD`;
      }
    }),
    ...mapMutations({
      addCommit: "addItem"
    }),
    ...mapActions({
      addDispatch: "items/addItem",
      changeSample: "sample/changeSample",
      setData: "user/setData"
    })
  },
  created(){
    window.start_component = this;
    if(!this.isAlreadyGotData){
      this.getData();
    }
    console.log("created in start component", this);
  },
  mounted(){
    console.log("mounted in start component", this);
  }
  ,
  beforeRouteEnter (route, redirect, next) {
    console.info("in beforeRouteEnter");
    //- window.alert("loading");
    next();
  }
}</script>